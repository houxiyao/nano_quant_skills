...
- Evidence: 以 `clickhouse://table?q=...` 为 source_url，excerpt 是关键字段摘要

所有连接为只读查询；失败时返回 `([], None)`。
"""
...
from .domain import Evidence, SourceType, now_iso
...
def _mk_evidence(table: str, query_tag: str, excerpt: str) -> Evidence | None:
    if not excerpt.strip():
        return None
    return Evidence(
        source_type=SourceType.DB,
        source_url=f"clickhouse://{table}?{query_tag}",
        retrieved_at=now_iso(),
        excerpt=excerpt[:600],
        title=f"ClickHouse.{table}",
    )


def open_connection() -> Connection:
    """获取 ClickHouse 连接。使用全局配置。"""
...
def probe_sw_peers(
    con: Connection, ts_code: str, limit: int = 10
...
def probe_company_overview(
    ...
    return r, _mk_evidence("stk_company", f"ts_code={ts_code}", excerpt)


def probe_managers(
    con: Connection, ts_code: str, limit: int = 20
) -> tuple[list[dict[str, Any]], Evidence | None]:
    rows = _rows_to_dicts(
        con.execute(
            """
...
def probe_cash_ratio(
    con: Connection, ts_code: str, years: int = 5
) -> tuple[list[dict[str, Any]], Evidence | None]:
    """净现比 = 经营性现金流净额 / 净利润。"""
    rows = _rows_to_dicts(
        con.execute(
    ...
    if not rows:
        return [], None
    values = [
        f"{r['end_date']}: 净现比={r['ocf_to_profit']}, 销售现金比={r['salescash_to_or']}"
        for r in rows
    ]
    excerpt = f"最近 {len(rows)} 年现金流质量 | " + " | ".join(values)
...
def probe_mainbz(
    con: Connection, ts_code: str, years: int = 3
) -> tuple[list[dict[str, Any]], Evidence | None]:
    rows = _rows_to_dicts(
        con.execute(
            """
...
def probe_pledge(
    con: Connection, ts_code: str, limit: int = 6
) -> tuple[list[dict[str, Any]], Evidence | None]:
    rows = _rows_to_dicts(
        con.execute(
            """
...
def probe_forecast(
    con: Connection, ts_code: str, limit: int = 4
) -> tuple[list[dict[str, Any]], Evidence | None]:
    rows = _rows_to_dicts(
        con.execute(
            """
